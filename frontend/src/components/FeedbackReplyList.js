...
function FeedbackReplyList({ feedbackId, onReplyAdded }) {
  ...
  const toggleReplyForm = (replyInfo = null) => {
    console.log(
      "Toggle reply form called:",
      replyInfo ? `Replying to ${replyInfo.userName}` : "Toggle main form"
    );
    if (replyInfo) {
      ...
      console.log(
        "Set replyingTo: null and toggled showReplyForm to:",
        !showReplyForm
      );
    }
  };
  ...
  const toggleDropdown = (replyId) => {
    console.log(
      `Toggle dropdown for reply ID: ${replyId}, current active: ${activeDropdown}`
    );
      ...
      >
          ...
          <div className="flex items-center">
            ...
            <div>
              <div className="flex items-center flex-wrap gap-1">
                ...
                {roleDisplay && (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${roleBadgeColor}`}
                  >
                    {roleDisplay}
                  </span>
                )}
        ...
        {!isEditing && (
          <div className="ml-10 mt-1">
            <button
              onClick={() =>
                toggleReplyForm({ id: reply.id, userName: reply.userFullName })
              }
  ...
  return (
    <div className="mt-4">
      {console.log(
        "FeedbackReplyList render - replies:",
        replies.length,
        "replyingTo:",
        replyingTo,
        "showReplyForm:",
      ...
      <div
        className={
          replies.length > 0
            ? "mt-4 border-t dark:border-gray-700 pt-3"
            : "mt-3 border-t dark:border-gray-700 pt-3"
        }
      >
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          {replies.length > 0 ? "Add new reply" : "First reply"}
        </h3>
