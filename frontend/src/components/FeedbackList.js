 ...
 */
function FeedbackList({ type, targetId, lessonId, showTitle = true, maxItems, requireAuth = false }) {
    const { user } = useAuth();
    ...
    try {
      ...
      setLoading(false);
        ...
        console.log(`Fetching feedbacks: type=${type}, targetId=${targetId}, lessonId=${lessonId}`);
        fetchFeedbacks();
    ...
    const renderStarRating = (rating) => {
        return (
            <div className="flex items-center">
                ...
                <span className="ml-1 text-sm text-gray-500 dark:text-gray-400">{rating}/5</span>
            </div>
        );
    };
    ...
    const getTitle = () => {
        switch (type) {
            case 'LESSON':
                return 'Phản hồi về bài học';
            case 'LECTURER':
                return 'Phản hồi về giảng viên';
            case 'SYSTEM':
                return 'Phản hồi về hệ thống';
            case 'USER':
                return 'Phản hồi về người dùng';
            default:
                return 'Phản hồi';
        }
    };
    ...
    const renderNoFeedbackMessage = () => {
        switch (type) {
            case 'LESSON':
                return 'Chưa có phản hồi nào về bài học này.';
            case 'LECTURER':
                return 'Chưa có phản hồi nào về giảng viên này.';
            case 'SYSTEM':
                return 'Chưa có phản hồi nào về hệ thống.';
            case 'USER':
                return 'Chưa có phản hồi nào về người dùng này.';
            default:
                return 'Chưa có phản hồi nào.';
        }
    };
    ...
    const toggleReplies = (feedbackId) => {
        console.log(`Toggle replies for feedback ID: ${feedbackId}, current expanded: ${expandedFeedback}`);
    ...
    const handleSaveEdit = async (feedbackId) => {
        if (editContent.trim().length < 10) {
            setSubmitError('Nội dung phản hồi phải có ít nhất 10 ký tự');
            return;
        }
        ...
        try {
            ...
            const updatedFeedback = await FeedbackService.updateFeedback(feedbackId, updatedFeedbackData);
            ...
            setSubmitError(
                err.response?.data?.error || 
                'Có lỗi xảy ra khi cập nhật phản hồi. Vui lòng thử lại sau.'
            );
        } finally {
    ...
    const handleDeleteFeedback = async (feedbackId) => {
        ...
        try {
            ...
            setFeedbacks(prevFeedbacks => {
                ...
                const updatedFeedbacks = prevFeedbacks.filter(feedback => feedback.id !== feedbackId);
            ...
            alert(
                err.response?.data?.error || 
                'Có lỗi xảy ra khi xóa phản hồi. Vui lòng thử lại sau.'
            );
        }
    };
    ...
    const toggleDropdown = (feedbackId) => {
        console.log(`Toggle dropdown for feedback ID: ${feedbackId}, current active: ${activeDropdown}`);
                ...
                <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center">
                        <img 
                            ...
                            alt={feedback.userFullName || 'Người dùng ẩn danh'} 
                            className="w-10 h-10 rounded-full mr-3 object-cover"
                        />
                        <div>
                            <div className="flex items-center flex-wrap gap-1">
                                <span className="font-medium text-gray-800 dark:text-gray-200">
                                    {feedback.userFullName || 'Người dùng ẩn danh'}
                                </span>
                                
                                {feedback.userRole && (
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${getRoleBadgeColor(feedback.userRole)}`}>
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        {canModifyFeedback(feedback) && (
                            <div className="relative" ref={el => dropdownRefs.current[feedback.id] = el}>
                                <button 
                                    onClick={(e) => {
                                        e.stopPropagation();
                    ...
                    <div className="mt-3">
                      ...
                      <button
                                    ...
                                    >
                                        <svg
                                            className={`w-6 h-6 ${
                                                star <= editRating ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'
                                            }`}
                                            fill="currentColor"
                                            viewBox="0 0 20 20"
                                            xmlns="http://www.w3.org/2000/svg"
                                ...
                                <span className="ml-2 text-gray-700 dark:text-gray-300">{editRating}/5</span>
                            </div>
                        </div>
                ...
                {expandedFeedback === feedback.id ? (
                    ...
                    <div className="mt-3">
                        ...
                        >
                            ...
                            {feedback.replyCount > 0 ? 
                                `Xem ${countTotalReplies(feedback.replies) || feedback.replyCount} phản hồi` : 
                                'Trả lời'}
                        </button>
                    </div>
                )}
        ...
        <div className="feedback-list">
            ...
            {maxItems && feedbacks.length > maxItems && (
                <div className="text-center mt-4">
                    <button
                        onClick={() => navigate(`/feedbacks/${type.toLowerCase()}/${targetId || lessonId}`)}
                        className="text-blue-600 dark:text-blue-400 hover:underline font-medium"
                    >
                        Xem tất cả {feedbacks.length} phản hồi
                    </button>
        </div>
      )}
    </div>
