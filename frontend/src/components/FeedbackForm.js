...
function FeedbackForm({ type, targetId, targetName, onSuccess }) {
  ...
  const getFormTitle = () => {
    switch (type) {
      case "LESSON":
        return `Give feedback on the lesson: ${targetName || targetId}`;
      case "LECTURER":
        return `Send feedback to the instructor: ${targetName || targetId}`;
      case "SYSTEM":
        return "Send feedback about the system";
      case "USER":
        return `Send user feedback: ${targetName || targetId}`;
      default:
  ...
  const handleSubmit = async (e) => {
    ...
    if (content.trim().length < 10) {
      setError("Response content must be at least 10 characters");
      return;
    }
    ...
    try {
      ...
      if (user) {
        setSuccess("Your comment has been sent successfully!");
      } else {
        setSuccess("Your anonymous feedback has been sent successfully!");
      }

      // Gọi callback nếu được truyền vào
      if (onSuccess && typeof onSuccess === "function") {
        console.log(
          "Calling onSuccess callback with new feedback:",
          newFeedback
        );
      ...
      setError(
        err.response?.data?.error ||
          ...
          "An error occurred while sending the response. Please try again later."
      );
    } finally {
      ...
      {!user && (
        <div className="p-4 mb-4 bg-blue-100 text-blue-700 rounded-md">
          <div className="flex items-center">
            ...
            <span>
              You are submitting feedback as <strong>anonymous</strong>.
              <span
              ...
              to send feedback with your name.
            </span>
          </div>
        </div>
      )}
