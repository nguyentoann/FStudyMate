...
const JokeNotification = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (isTargetUser) {
        console.log('JokeNotification: Target user found, showing notification');
    ...
    <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-[1000] animate-bounce-in">
      <div className="bg-white rounded-lg shadow-2xl overflow-hidden max-w-md">
        <div className="relative">
          ...
          {/* Joke image */}
          <img 
            src={`${API_URL}/StudentImages/joke1.jpg`}
            alt="Birthday Joke"
            className="w-full object-cover"
            style={{ maxHeight: '300px' }}
            onError={(e) => {
              console.log('JokeNotification: Image failed to load, using fallback');
              e.target.onerror = null;
              e.target.src = '/images/default-avatar.svg'; // Fallback image
            }}
          />
        </div>
        ...
        <div className="p-4 text-center">
          <h3 className="text-xl font-bold text-indigo-600 mb-2">Sinh Nhat dui de:)))))</h3>
        </div>
      </div>
    </div>
  );
