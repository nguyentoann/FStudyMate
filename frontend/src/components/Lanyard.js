    ...
    <div className="lanyard-wrapper">
      ...
      >
        ...
        <Physics gravity={gravity} timeStep={1 / 60}>
          <Band backendBaseUrl={backendBaseUrl} />
        </Physics>
...
function Band({ maxSpeed = 50, minSpeed = 0, backendBaseUrl }) {
  const band = useRef(), fixed = useRef(), j1 = useRef(), j2 = useRef(), j3 = useRef(), card = useRef();
  const vec = new THREE.Vector3(), ang = new THREE.Vector3(), rot = new THREE.Vector3(), dir = new THREE.Vector3();
  const segmentProps = { type: 'dynamic', canSleep: true, colliders: false, angularDamping: 0.5, linearDamping: 1.0 };
  const cardUrl = `${backendBaseUrl}/api/StudentImages/card.glb`;
  const lanyardUrl = `${backendBaseUrl}/api/StudentImages/lanyard.png`;
