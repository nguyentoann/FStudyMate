...
function LecturerFeedbackForm({ onSuccess }) {
  ...
  const fetchLecturers = async (department) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/lecturer-form/lecturers?department=${department}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const selectedLecturerObj = lecturers.find(
        (lecturer) => lecturer.lecturerId === selectedLecturer
      );
      ...
      if (user) {
        setSuccess(
          `Phản hồi của bạn về giảng viên ${
            selectedLecturerObj?.fullName || selectedLecturer
          } đã được gửi thành công!`
        );
      } else {
        setSuccess(
          `Phản hồi ẩn danh của bạn về giảng viên ${
            selectedLecturerObj?.fullName || selectedLecturer
          } đã được gửi thành công!`
        );
      }
                ...
                <div key={lecturer.lecturerId} className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Department: {lecturer.department || "Not updated"}
                    </p>
                    {lecturer.specializations && (
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Specialization: {lecturer.specializations}
                      </p>
                    )}
                  </div>
                </div>
