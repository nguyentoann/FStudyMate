...
function FeedbackPage() {
  const { user } = useAuth();
    ...
    const [feedbackType, setFeedbackType] = useState(queryParams.get('type') || 'SYSTEM');
    const [targetId, setTargetId] = useState(queryParams.get('targetId') || 'system');
    const [targetName, setTargetName] = useState(queryParams.get('targetName') || 'Hệ thống');
    ...
    const handleFeedbackSubmitSuccess = (feedback) => {
        ...
        setRefreshKey(prevKey => {
            ...
            console.log(`Updating refreshKey from ${prevKey} to ${newKey} to trigger FeedbackList refresh`);
            return newKey;
        });
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <label className="flex items-center space-x-2 p-2 border rounded-md cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            />
                            <span className="text-gray-700 dark:text-gray-300">Hệ thống</span>
                        </label>
                        ...
                        <label className="flex items-center space-x-2 p-2 border rounded-md cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                            <input 
                                ...
                                onChange={handleTypeChange}
                            />
                            <span className="text-gray-700 dark:text-gray-300">Bài học</span>
                        </label>
                        ...
                        <label className="flex items-center space-x-2 p-2 border rounded-md cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                            <input 
                                ...
                                onChange={handleTypeChange}
                            />
                            <span className="text-gray-700 dark:text-gray-300">Giảng viên</span>
                        </label>
                        ...
                        <label className="flex items-center space-x-2 p-2 border rounded-md cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                            <input 
                                ...
                                onChange={handleTypeChange}
                            />
                            <span className="text-gray-700 dark:text-gray-300">Người dùng</span>
                        </label>
                    </div>
                    
                    {/* Input cho targetId và targetName nếu không phải SYSTEM và không phải LESSON và không phải LECTURER */}
                    {feedbackType !== 'SYSTEM' && feedbackType !== 'LESSON' && feedbackType !== 'LECTURER' && (
