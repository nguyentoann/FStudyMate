...
function TeachingScheduleManager() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        const [roomsRes, subjectsRes, classesRes, lecturersRes, termsRes] =
          await Promise.all([
            api.get("/api/rooms"),
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      if (scheduleData.length > 0) {
        ...
        console.log(
          "Schedule time format example:",
          `startTime: ${scheduleData[0].startTime}, endTime: ${scheduleData[0].endTime}`
        );
      } else {
        console.warn(
          "No schedule data returned from API or data is not an array"
        );
      }
  ...
  const handleModalSave = async (formData) => {
    try {
      ...
      const conflictResponse = await api.post(
        "/api/schedule/class/validate-conflicts",
        payload
      );
      ...
      if (
        ...
        if (conflicts.lecturerConflict)
          conflictMessages.push("Lecturer has a schedule conflict");
        if (conflicts.classConflict)
          conflictMessages.push("Class has a schedule conflict");
        if (conflicts.roomConflict)
          conflictMessages.push("Room is already occupied at this time");

        toast.error(
          `Schedule conflicts detected: ${conflictMessages.join(", ")}`
        );
        return;
      }
      ...
      if (formData.isEdit && formData.id) {
        ...
        // For recurring schedules, use a different endpoint
          ...
          toast.success(
            `Recurring schedule created for ${formData.recurrenceCount} occurrences`
          );
        }
      ...
      if (error.response && error.response.status === 409) {
        toast.error(
          "Schedule conflict detected. Please check the time and resources."
        );
      } else {
        toast.error(
  ...
  const generateWeekMatrix = useCallback(() => {
      ...
      WEEKDAYS.map((_, dayIdx) => {
        ...
        const daySchedules = schedulesArray.filter((s) => {
          ...
          const fuzzyStartMatch =
            normalizedStartTime.startsWith(timeSlot.start) ||
            timeSlot.start.startsWith(normalizedStartTime);
          const fuzzyEndMatch =
            normalizedEndTime.startsWith(timeSlot.end) ||
    ...
    const hasSchedules = matrix.some((row) =>
      ...
      return SLOTS.map((slot, slotIdx) =>
        WEEKDAYS.map((_, dayIdx) => {
          ...
          if (
            ...
            schedulesByDay[dayIdx].length > 0
          ) {
            ...
            onClick={async () => {
              try {
                ...
                if (Array.isArray(debugData) && debugData.length > 0) {
                  alert(
                    `Found ${debugData.length} schedules. Check console for details.`
                  );
              ...
              <tr key={slot}>
                <td className="slot-info">
                  ...
                  <div className="time-range">
                    {TIME_SLOTS[slot] && (
                      <small>
                        {TIME_SLOTS[slot].start} - {TIME_SLOTS[slot].end}
                      </small>
                    )}
                  </div>
                </td>
                      ...
                      {cellSchedules
                        ? cellSchedules.map((schedule) => (
                            <div
                              ...
                              style={{
                                borderColor:
                                  STATUS_COLORS[schedule.status] || "#ccc",
                                borderLeftWidth: "4px",
                              }}
                            >
                              ...
                              <div className="schedule-room">
                                {schedule.room ? schedule.room.name : "No Room"}
      ...
      >
        ...
        <details>
          ...
          <div style={{ maxHeight: "200px", overflow: "auto" }}>
            {rooms.map((room, idx) => (
              <div key={idx}>
                Room {idx + 1}: {room.name} - {room.location} (Capacity:{" "}
                {room.capacity})
              </div>
            ))}
          </div>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              {conflicts.classConflict && (
                <div className="conflict-warning">
                  <i className="fas fa-exclamation-triangle"></i> This class has
                  another schedule at this time
                </div>
              )}
            </div>
          </div>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              {conflicts.lecturerConflict && (
                <div className="conflict-warning">
                  <i className="fas fa-exclamation-triangle"></i> This lecturer
                  has another schedule at this time
                </div>
              )}
            </div>
            ...
            <div className="form-group">
              ...
              {conflicts.roomConflict && (
                <div className="conflict-warning">
                  <i className="fas fa-exclamation-triangle"></i> This room is
              ...
              <div className="radio-group">
                <label>
                  <input
                    ...
                    onChange={() =>
                      setForm({ ...form, isOneTimeChange: false })
                    }
                  />
                  Regular Schedule
                </label>
              ...
              >
                {SLOTS.map((slot) => (
                  <option key={slot} value={slot}>
                    Slot {slot} ({timeSlots[slot]?.start || ""} -{" "}
                    {timeSlots[slot]?.end || ""})
                  </option>
                ))}
              </select>
