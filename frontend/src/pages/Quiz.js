...
const TeacherAvatar = () => {
  ...
  const updateHeadRotation = () => {
    if (headRef.current) {
      headRef.current.style.transform = `perspective(500px) rotateX(${-rotation.x}deg) rotateY(${
        rotation.y
      }deg)`;
    }
  };
        ...
        <div className="relative w-full h-full">
          ...
          >
            <div className="relative w-3 h-3">
              <div
                className={`absolute w-full h-full bg-gray-800 rounded-full ${
...
const Quiz = () => {
  const { maMon, maDe, quizTakenId } = useParams();

  // If we have a quizTakenId, we're continuing an existing attempt
  if (quizTakenId) {
    return <QuizAttemptContinue quizTakenId={quizTakenId} />;
  }
...
// Component for continuing a quiz attempt
const QuizAttemptContinue = ({ quizTakenId }) => {
    ...
    const loadQuizAttempt = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          quizData = data.data;
        } else if (data.success && data.quizTaken) {
          // Format 2: { success: true, quizTaken: {...} }
          quizData = data.quizTaken;
        } else if (data.status !== undefined) {
          // Format 3: Direct quiz data { id: ..., status: ..., quiz: {...} }
          quizData = data;
        } else {
          console.error("Failed to load quiz attempt:", data);
        ...
        if (quizData.quizId) {
          ...
          try {
            ...
            if (quizDetailsData.success && quizDetailsData.data) {
              quiz = quizDetailsData.data;
            } else if (quizDetailsData.quiz) {
              quiz = quizDetailsData.quiz;
            } else {
            ...
            if (quiz && quiz.maMon && quiz.maDe) {
              ...
              console.error("Quiz details don't have maMon and maDe:", quiz);
              setError("Quiz details incomplete. Please try again.");
            }
          } catch (quizError) {
          ...
          console.error("No quiz data found in response:", quizData);
          setError("No quiz information found. Please try again.");
        }
      } catch (err) {
...
const QuizComponent = ({ maMon, maDe }) => {
  ...
  const alertSoundRef = useRef(
    new Audio("http://toandz.ddns.net/fstudy/sound/quack.mp3")
  );
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      try {
        ...
        if (metadataResponse.success && metadataResponse.data) {
          ...
          console.error("Invalid metadata format:", metadataResponse);
          metadata = {
            title: `${maMon} - ${maDe}`,
        ...
        // Initialize quiz session if quiz ID is available and we're not continuing an existing quiz
        if (metadata.id && !continuing) {
        ...
        if (continuing && quizTakenId) {
          ...
          try {
            ...
            let attemptData;
            if (data.success && data.data) {
              attemptData = data.data;
            } else if (data.success && data.quizTaken) {
              attemptData = data.quizTaken;
            } else if (data.status !== undefined) {
              // Direct data format
              attemptData = data;
            }
            ...
            console.error("Failed to load quiz attempt data:", loadError);
          }
        }
        ...
        if (Array.isArray(questionsResponse)) {
          ...
          console.error("Invalid questions data format:", questionsResponse);
          questionData = [];
        }
    ...
    fetchQuizData();
  }, [maMon, maDe, random, timed, continuing, quizTakenId]);

  // Save state when it changes
  useEffect(() => {
    if (questions.length > 0) {
      // Only save if questions are loaded
      const stateToSave = {
        currentIndex,
        selectedAnswers,
  ...
  useEffect(() => {
    const handleBeforeNavigate = (e) => {
      if (!showResults) {
        if (
          !window.confirm(
            "Bạn có chắc muốn rời khỏi trang? Tiến độ làm bài sẽ được lưu lại."
          )
        ) {
          e.preventDefault();
        }
  ...
  useEffect(() => {
    ...
    const handleContextMenu = (e) => {
      if (
        ...
        quizContainerRef.current.contains(e.target)
      ) {
        e.preventDefault();
        setShowCopyAlert(true);
        alertSoundRef.current
  ...
  const retryCamera = () => {
    ...
    navigator.mediaDevices
      ...
      .catch((err) => {
        ...
        alert(
          "Bạn cần cho phép truy cập camera để tiếp tục làm bài thi. Camera giúp giám sát quá trình làm bài để đảm bảo tính công bằng."
        );
      });
  };
      ...
      if (!quizTakenId) {
        ...
        if (!startResponse.success) {
          const error = new Error(
            "Failed to start quiz session: " + startResponse.message
          );
          if (DEBUG_QUIZ_SUBMISSIONS) console.error("DEBUG:", error.message);
          throw error;
        }
      ...
      if (question && (question.correct || question.correctAnswer)) {
        ...
        if (correctArray.length > 1) {
          ...
          const penaltyScore = Math.min(
            rawScore,
            Math.max(0, rawScore - incorrectCount * penaltyPerIncorrect)
          );
        ...
        // For single choice questions
          ...
          if (
            ...
            correctArray.includes(selectedArray[0])
          ) {
  ...
  const handleCheckAnswer = React.useCallback(() => {
    ...
    if (correctArray.length > 1) {
      ...
      isCorrect =
        selectedArray.length === 1 && correctArray.includes(selectedArray[0]);
  ...
  const ZoomableImage = React.memo(
    ({ src, alt, className, questionId, questionImg, currentQuestion }) => {
      ...
      return (
        <img
          ...
          onError={(e) => {
            ...
            if (!hasLoaded) {
              console.log(
                `Failed to load image with path ${imagePath}. Trying alternative path...`
              );
            }
            ...
            if (e.target.src.includes(`${maMon}/${maDe}/`)) {
              const filename = ensureExtension(questionImg);
              e.target.src = `${API_URL}/images/direct?path=quiz/${
                quiz_id || "default"
              }/${filename}`;
              if (!hasLoaded) {
    ...
    (prevProps, nextProps) => {
      ...
      return (
        prevProps.questionImg === nextProps.questionImg &&
        ...
        <div className="flex flex-col md:flex-row flex-grow">
          ...
          <div className="w-full md:w-3/5 p-4 overflow-y-auto max-h-screen">
            {questionResults.map((result, index) => {
              ...
              const bgColorClass = result.fullScore
                ? "bg-green-50 border-green-200"
                : "bg-red-50 border-red-200";
              const textColorClass = result.fullScore
                ? "text-green-800"
                : "text-red-800";
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center mb-2">
                      ...
                      >
                        {Math.round(result.earnedPoints)} /{" "}
                        {result.questionPoints} points
                      </div>
                    </div>
                            ...
                            return (
                              <code
                                className={`${
                                  inline
                                    ? darkMode
                                      ? "bg-gray-700 text-blue-300 px-1 rounded"
                                      : "bg-gray-100 text-blue-600 px-1 rounded"
                                    : darkMode
                                    ? "block bg-gray-700 p-2 rounded border border-gray-600"
                                    : "block bg-gray-50 p-2 rounded border border-gray-200"
                                } ${className || ""}`}
                                {...props}
                              >
                                {children}
                    ...
                    {result.isMultipleChoice && (
                      ...
                      >
                        Multiple Choice: {result.correctCount}/
                        {result.totalCorrect} correct answers selected
                        {result.correctCount > 0 &&
                          result.correctCount < result.totalCorrect && (
                            <span>
                              {" "}
                              = {(result.questionScoreRatio * 100).toFixed()}%
                              partial credit
                            </span>
                          )}
                      </div>
      ...
      >
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div
            ...
            className="border-2 border-red-500 red-zone rounded-xl overflow-hidden"
          >
            <div
              className={`${
                darkMode ? "bg-gray-800" : "bg-white"
            ...
            >
              <div className="bg-gradient-to-r from-indigo-600 to-blue-500 px-6 py-4 text-white flex justify-between items-center animate-fade-in-up">
                ...
                {timed && (
                  <div
                    ...
                    } font-mono text-xl animate-slide-in-right`}
                  >
              ...
              <div className="p-6 md:p-8 animate-fade-in-up">
                ...
                <div
                  ...
                  } rounded-full h-3 mb-4 overflow-hidden shadow-inner`}
                >
                  <div
                        ...
                        {(() => {
                          ...
                          if (isMultipleChoice) {
                            ...
                            if (Array.isArray(currentQuestion?.correct)) {
                              ...
                              correctCount =
                                currentQuestion.correct.split(/[,;]/).length;
                            }

                            return (
                              <span className="text-green-500 font-normal ml-2">
                                Multiple Choice, choose {correctCount} correct
                                answer{correctCount > 1 ? "s" : ""}!
                              </span>
                            );
                          }
                          return null;
                      ...
                      {hasValidAnswers ? (
                        currentQuestion.answers.map((answer, answerIndex) => {
                          ...
                          const isCorrectAnswer =
                            isChecked &&
                            checkResult &&
                            answer === checkResult.correctAnswer;
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-full mr-3 flex items-center justify-center transition-all duration-300 hover:scale-110 ${
                                  isSelected
                    ...
                    <div className="text-center font-bold text-lg mb-2">
                      {checkResult.isCorrect
                        ...
                        : checkResult.isMultipleChoice &&
                          checkResult.correctCount > 0
                        ? "Partially Correct!"
                        : "Incorrect!"}

                      {checkResult.isMultipleChoice && (
                        <div className="text-sm font-normal mt-1">
                          {checkResult.correctCount}/{checkResult.totalCorrect}{" "}
                          correct answers selected
                          {checkResult.partialScore > 0 &&
                            checkResult.partialScore < 1 && (
                              <span className="ml-2">
                                ({(checkResult.partialScore * 100).toFixed(0)}%
                                score)
                              </span>
                            )}
                        </div>
                    ...
                    {!checkResult.isCorrect && (
                      <div className="mt-2">
                        <div className="font-medium">
                          ...
                          checkResult.correctAnswer.length > 1
                            ? "s"
                            : ""}
                          :
                        </div>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded flex items-center transition-all duration-300 hover:scale-105 ${
                      currentIndex === 0
                        ...
                        : darkMode
                        ? "bg-gray-700 text-gray-200 hover:bg-gray-600 animate-slide-in-left"
          ...
          {/* <div className="fixed bottom-8 left-8 z-30">
            <div className="relative">
              <video
                ...
                playsInline
                muted
                className={`rounded-lg border-4 border-red-600 shadow-lg w-64 h-48 object-cover ${
                  webcamActive ? "opacity-100" : "opacity-70"
                }`}
              />
    ...
    if (selectedSubject) {
      const fetchExamCodes = async () => {
        try {
          ...
          setError(
            `Failed to load exam codes for ${selectedSubject}. ${error.message}`
          );
          setLoading(false);
        }
      };
          ...
          (subject, index) =>
            subject && (
              <div
                ...
                } p-6 rounded-lg cursor-pointer transition-colors border`}
  ...
  const renderExamCodes = () => {
    ...
    if (!Array.isArray(examCodes) || examCodes.length === 0) {
      return (
        ...
        >
          <p>
            No exams available for this subject. Please try another subject.
          </p>
        </div>
      );
    }
          ...
          return (
            examCode && (
              ...
              >
                    ...
                    <>
                      ...
                      {metadata.description && (
                        ...
                        >
                          ...
                          <p className="border-l-2 pl-3 border-gray-400">
                            {metadata.description}
                          </p>
                        </div>
                      )}
                ...
                >
                  <span className="text-sm font-medium text-indigo-500 hover:text-indigo-400 transition-colors">
                    Click to start exam
                  </span>
                </div>
              </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          >
            <h1 className="text-2xl font-bold mb-6 border-b pb-4">
              {selectedSubject
                ? `Select an Exam for ${selectedSubject}`
                : "Select a Subject"}
            </h1>
...
const LeaderboardComponent = ({ leaderboardData, score, darkMode }) => {
  ...
  const currentUsername =
    currentUser.username || localStorage.getItem("username") || "You";
  ...
  if (!currentUserEntry && score > 0) {
    ...
    const estimatedRank =
      sortedLeaderboard.findIndex((entry) => (entry.score || 0) < score) + 1;

    // If couldn't find a position (user's score is lowest)
    const finalRank =
      estimatedRank > 0 ? estimatedRank : sortedLeaderboard.length + 1;
    ...
    return (
      <div className={style.containerClass}>
        <div className="relative inline-block">
          <div
            className={`${style.imageClass} rounded-full overflow-hidden border-2 ${style.borderClass} bg-gray-700 flex items-center justify-center`}
          >
            <span className="text-2xl text-gray-500">?</span>
          </div>
          <div
            className={`absolute -bottom-2 left-1/2 transform -translate-x-1/2 ${style.badgeClass} rounded-full ${style.badgeSize} flex items-center justify-center text-sm font-bold`}
          >
            {position}
          </div>
        </div>
        ...
        {topUsers.length > 0 && (
          <div className="absolute top-2 left-1/2 transform -translate-x-1/2">
            ...
            >
              <path
                fillRule="evenodd"
                d="M10 2a1 1 0 01.894.553l2.991 5.982a1 1 0 01.068.756l-1.07 3.756a1 1 0 01-.962.753H8.079a1 1 0 01-.962-.753l-1.07-3.756a1 1 0 01.068-.756l2.991-5.982A1 1 0 0110 2zm0-2a3 3 0 00-2.683 1.658L4.336 7.256a3 3 0 00-.204 2.268l1.071 3.755A3 3 0 008.08 16h3.84a3 3 0 002.878-2.721l1.07-3.755a3 3 0 00-.204-2.268L12.683 1.658A3 3 0 0010 0z"
              ></path>
            </svg>
          </div>
        )}
            ...
            <div className="relative inline-block">
              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-indigo-500">
                <img
                  src={
                    topUsers[1].avatar ||
                    getDefaultAvatar(topUsers[1].userId, topUsers[1].username)
                  }
                  alt="2nd place"
                  className="w-full h-full object-cover"
                />
              </div>
              <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 bg-indigo-600 rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold">
                2
              </div>
            </div>
            <p className="mt-2 font-medium">
              {topUsers[1].name || topUsers[1].username}
            </p>
            ...
            <div className="relative inline-block">
              <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-yellow-500 z-10">
                <img
                  src={
                    topUsers[0].avatar ||
                    getDefaultAvatar(topUsers[0].userId, topUsers[0].username)
                  }
                  alt="1st place"
                  className="w-full h-full object-cover"
                />
              </div>
              <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 bg-yellow-500 rounded-full w-7 h-7 flex items-center justify-center text-sm font-bold">
                1
              </div>
            </div>
            <p className="mt-2 font-medium">
              {topUsers[0].name || topUsers[0].username}
            </p>
            ...
            <div className="relative inline-block">
              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-green-500">
                <img
                  src={
                    topUsers[2].avatar ||
                    getDefaultAvatar(topUsers[2].userId, topUsers[2].username)
                  }
                  alt="3rd place"
                  className="w-full h-full object-cover"
                />
              </div>
              <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 bg-green-600 rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold">
                3
              </div>
            </div>
            <p className="mt-2 font-medium">
              {topUsers[2].name || topUsers[2].username}
            </p>
      ...
      <div className="divide-y divide-gray-800">
        ...
        {regularUsers.length > 0 ? (
          regularUsers.map((entry) => (
            ...
            >
              <div className="w-8 text-right mr-3 text-gray-500">
                {entry.rank}
              </div>
              <div className="w-8 h-8 rounded-full overflow-hidden mr-3">
                <img
                  src={
                    entry.avatar ||
                    getDefaultAvatar(entry.userId, entry.username)
                  }
