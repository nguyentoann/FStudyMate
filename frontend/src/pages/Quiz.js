        ...
        <div className="flex flex-col md:flex-row flex-grow">
          ...
          <div className="w-full md:w-3/5 p-4 overflow-y-auto max-h-screen">
            {questionResults.map((result, index) => {
              // Determine background color based on correctness in dark mode and light mode
              const bgColorClass = darkMode
                ? (result.fullScore ? 'bg-green-900/30 border-green-700' : 'bg-red-900/30 border-red-700')
                : (result.fullScore ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200');
              
              const textColorClass = darkMode
                ? (result.fullScore ? 'text-green-400' : 'text-red-400') 
                : (result.fullScore ? 'text-green-800' : 'text-red-800');
              
              return (
                <div key={`result-${result.id || index}`} className={`mb-6 rounded-lg overflow-hidden ${bgColorClass} border shadow-md`}>
                  <div className="p-5">
                    {/* Question number and points */}
                    <div className="flex justify-between items-center mb-4">
                      <div className={`text-lg md:text-xl font-semibold ${textColorClass}`}>
                        Câu {index + 1}:
                      </div>
                      <div className="text-sm font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300 py-1 px-3 rounded-full">
                        {Math.round(result.earnedPoints)} / {result.questionPoints} points
                      </div>
                    </div>
                    
                    {/* Question text */}
                    <div className={`${darkMode ? 'text-gray-300' : 'text-gray-800'} mb-4`}>
                      <ReactMarkdown
                        components={{
                          code: ({node, inline, className, children, ...props}) => {
                            return (
                              <code className={`${inline 
                                ? (darkMode ? 'bg-gray-800 text-blue-400' : 'bg-gray-100 text-blue-600') + ' px-1 rounded' 
                                : (darkMode ? 'block bg-gray-800 p-2 rounded border border-gray-700' : 'block bg-gray-50 p-2 rounded border border-gray-200')} ${className || ''}`} {...props}>
                                {children}
                              </code>
                            )
                          }
                    ...
                    {/* User answer */}
                    <div className={`${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>
                    ...
                    {result.explanation && (
                      <div className={`mt-4 pt-3 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                        <div className={`font-medium text-lg ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Giải thích:</div>
            ...
            <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg overflow-hidden`}>
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="flex justify-between text-sm text-gray-600 mb-6">
                  <div className={`text-base font-medium ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>Câu {currentIndex + 1} / {questions.length}</div>
                  ...
                  <div className="lg:flex-1">
                    ...
                    <div className="mb-4 flex justify-between items-center">
                      <div className="font-bold text-xl flex items-center">
                        <span className={`${darkMode ? 'text-blue-400' : 'text-blue-600'} mr-2 text-2xl`}>
                          Câu {currentIndex + 1}
                        </span>
                        ...
                        {(() => {
                          ...
                          if (isMultipleChoice) {
                            ...
                            return (
                              <span className={`${darkMode ? 'text-green-400' : 'text-green-500'} font-normal ml-2`}>
                      ...
                      {/* Display points */}
                      <div className={`text-sm font-medium px-3 py-1 ${darkMode ? 'bg-amber-900/50 text-amber-300' : 'bg-blue-100 text-blue-800'} rounded-full`}>
                ...
                {isChecked && checkResult && (
                  <div className={`mt-4 p-4 rounded-lg ${
                    checkResult.isCorrect 
                      ? (darkMode ? 'bg-green-900/40 border border-green-700 text-green-300' : 'bg-green-100 text-green-700')
                      : (darkMode ? 'bg-red-900/40 border border-red-700 text-red-300' : 'bg-red-100 text-red-700')
                  }`}>
