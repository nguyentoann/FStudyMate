...
public interface NotificationRecipientRepository extends JpaRepository<NotificationRecipient, Integer> {
    ...
    @Query("SELECT nr FROM NotificationRecipient nr WHERE nr.recipient.id = :recipientId AND nr.notification.isUnsent = false ORDER BY nr.notification.createdAt DESC")
    ...
    @Query("SELECT nr FROM NotificationRecipient nr WHERE nr.recipient.id = :recipientId AND nr.isRead = false AND nr.notification.isUnsent = false ORDER BY nr.notification.createdAt DESC")
    ...
    @Query("SELECT COUNT(nr) FROM NotificationRecipient nr WHERE nr.recipient.id = :recipientId AND nr.isRead = false AND nr.notification.isUnsent = false")
